...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (path.includes('index.html') || path.endsWith('/')) {
        ...
        // Load data for dashboard/network pages
...
async function fetchMembersFromSupabase() {
    try {
        ...
        showNotification("Gagal memuat data server.");
        return [];
    }
}
...
async function addMember() {
    ...
    if (error) {
        // Retry with capitalized keys just in case
        ...
        if (err2) return showNotification("Gagal: " + err2.message);
    }
...
function initDashboard() {
    ...
    document.getElementById('saveEditButton').onclick = saveEditedMember;
    document.getElementById('startAuditButton').onclick = runAudit;
}
...
async function saveEditedMember() {
    ...
    if(!error) {
        if(origUid !== newUid) {
            // Update children uplines if ID changed
...
function initNetwork() {
    ...
    document.getElementById('downloadNetworkButton').onclick = () => {
...
function runAudit() {
    ...
    setTimeout(() => {
        ...
        allMembersCache.forEach(m => {
            if(m.uid === target) { valid++; return; } // Skip root
